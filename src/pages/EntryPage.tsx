  ...
  return (
    <div className="min-h-[70vh] flex flex-col justify-between">
          ...
          {steps.map((s, i) => (
            <div key={s.id} className="flex items-center gap-2">
              <div className={`w-8 h-8 rounded-full flex items-center justify-center text-[12px] font-bold transition-all duration-500 ${
                i < step ? 'bg-esmeralda text-gray-900' :
                i === step ? 'bg-esmeralda/20 text-esmeralda border-2 border-esmeralda' :
                'bg-white/80 text-gray-900/20 border border-gray-200'
              }`}>
        ...
        <AnimatePresence mode="wait">
          {step === 0 && (
            ...
            >
              ...
              <div className="text-center">
                ...
                <h2 className="text-[24px] sm:text-[28px] font-extrabold text-gray-900 leading-tight">
                  Cuéntanos qué te pasa
                </h2>
                <p className="text-[14px] text-gray-900/40 mt-2 max-w-[300px] mx-auto">
              ...
              <textarea
                ...
                className="w-full h-40 px-5 py-4 rounded-2xl bg-white/80 border border-gray-200 text-gray-900 text-[15px] placeholder:text-gray-900/20 resize-none focus:border-esmeralda/40 focus:shadow-[0_0_30px_rgba(16,185,129,0.06)] transition-all leading-relaxed outline-none"
              />
            </motion.div>
          )}
          ...
          {step === 1 && (
            ...
            >
              ...
              <div className="border-2 border-dashed border-gray-200 rounded-3xl p-10 text-center hover:border-esmeralda/30 hover:bg-esmeralda/[0.02] transition-all duration-500 cursor-pointer group">
                <div className="w-14 h-14 rounded-full bg-gradient-to-br from-esmeralda/10 to-teal-400/5 border border-gray-200 flex items-center justify-center mx-auto mb-4 group-hover:border-esmeralda/30 group-hover:scale-110 transition-all duration-500">
                  <Upload size={22} className="text-gray-900/25 group-hover:text-esmeralda transition-colors" />
                </div>
                <p className="text-[14px] text-gray-900/35 group-hover:text-gray-900/60 transition-colors font-medium">
                  Toca para subir archivos
                </p>
                <p className="text-[11px] text-gray-900/15 mt-2">
              ...
              <button
                onClick={next}
                className="w-full py-3 text-center text-[13px] text-gray-900/30 hover:text-esmeralda transition-colors"
              >
                Omitir este paso →
              </button>
            </motion.div>
          ...
          {step === 2 && (
            ...
            >
              ...
              <input
                ...
                className="w-full h-14 px-5 rounded-2xl bg-white/80 border border-gray-200 text-gray-900 text-[16px] placeholder:text-gray-900/20 focus:border-esmeralda/40 focus:shadow-[0_0_30px_rgba(16,185,129,0.06)] transition-all outline-none text-center"
              />
            </motion.div>
          )}
        </AnimatePresence>
      ...
      <div className="mt-8 space-y-3">
        <button
          onClick={next}
          className="cta-glow w-full h-[56px] rounded-2xl bg-gradient-to-r from-esmeralda via-emerald-500 to-teal-500 text-gray-900 font-bold text-[15px] flex items-center justify-center gap-2.5 hover:-translate-y-[1px] active:translate-y-0 transition-transform"
        >
        ...
        {step > 0 && (
          <button
            onClick={back}
            className="w-full py-2 text-[13px] text-gray-900/25 hover:text-gray-900/50 transition-colors"
          >
            ← Volver
          </button>
        )}
        ...
        {step === 0 && (
          <p className="text-center text-[11px] text-gray-900/15 tracking-wide">
