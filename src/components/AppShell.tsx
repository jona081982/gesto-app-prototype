    ...
    <div className="min-h-screen relative">
      ...
      <header className="fixed top-0 left-0 right-0 z-50 px-4 pt-4">
        <div className="max-w-lg mx-auto glass-card rounded-2xl px-5 h-16 flex items-center justify-between">
          <img src="/images/logo-gestos-bv.png" alt="Gesto Copropiedad" className="h-9 w-auto invert" />
      ...
      <nav className="fixed top-[88px] left-0 right-0 z-40 px-4">
        <div className="max-w-lg mx-auto bg-white/70 backdrop-blur-md rounded-xl border border-gray-200/50 px-2.5 py-1.5 flex items-center gap-0.5 overflow-x-auto shadow-sm">
          <span className="text-[7px] font-mono text-esmeralda/50 mr-1.5 shrink-0 uppercase tracking-widest">Dev</span>
          {navItems.map((item) => (
            <Link
              key={item.path}
              to={item.path}
              className={`shrink-0 text-[9px] px-2 py-1 rounded-md transition-all ${
                pathname === item.path
                  ? 'bg-esmeralda/15 text-esmeralda font-semibold'
